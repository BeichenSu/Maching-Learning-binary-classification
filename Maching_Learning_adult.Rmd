...
This turns out we want smallest lambda, so that I pick lambda = 0 for this logistic regression model. Now I build the model. The run time is 47.98 seconds.
...
### 2.1.4 Performance of the model on local
...
### 2.1.4 Performance of the model on h2o
```{r}
library(h2o)
h2o.init(nthreads=-1)
dx = h2o.importFile("/Users/beichensu/Documents/STAT418/Stat 418/wk-05-ML/HW3/mydata.csv")
...
system.time({
  md <- h2o.glm(x = Xnames, y = "income", training_frame = dx_train, 
                family = "binomial", alpha = 1, lambda_search = TRUE, nfolds = 5)
})
md@model$lambda_best
h2o.auc(h2o.performance(md,dx_test))
```
As the best lambda search gives a really small lambda, it's reasonbale to set the lambda = 0 to fit the model. It only took 11.5 second to complete the search and it's 4 time faster than the local cv.glmnet which took 47.98 seconds. Now build a logistic regression model with h2o.
...
h2o.auc(h2o.performance(md, dx_test))
```
This model gives auc at 0.906, a little bit better than glmnet.
...
### 2.2.6 Random forest on h2o
For this random forest model on h2o I will employ a validation set and early stopping for time saving and avoiding overfitting.
```{r}
...
system.time({
  md <- h2o.randomForest(x = Xnames, y = "income", training_frame = dx_train, 
                         validation_frame = dx_valid, stopping_metric = "AUC", 
                         stopping_rounds = 5, stopping_tolerance = 0.001, ntrees = 500)
})
plot(md)
h2o.auc(h2o.performance(md,dx_test))
plot(h2o.performance(md))
```

As the plot suggests, 100 trees should be enough to avoid over-training. And the auc at 0.9148 is quite nice.
...
md = gbm(trainSet$income ~ ., data = trainSet, distribution = "bernoulli",
          n.trees = 200, interaction.depth = 5, shrinkage = 0.6, cv.folds = 5)
gbm.perf(md, plot.it = TRUE)
```

This model suggests that 25 trees should be sufficient to avoid overfitting.

Now I tune the depth up to 10, and learning ratio as the same.
```{r}
md = gbm(trainSet$income ~ ., data = trainSet, distribution = "bernoulli",
          n.trees = 200, interaction.depth = 10, shrinkage = 0.6, cv.folds = 5)
gbm.perf(md, plot.it = TRUE)
```

It turns out that only 14 trees are needed, which implies that increasing in depths results in less tree.

Below I keep the depth at 5 and tune the learning ratio down to 0.3.
```{r}
md = gbm(trainSet$income ~ ., data = trainSet, distribution = "bernoulli",
          n.trees = 200, interaction.depth = 5, shrinkage = 0.3, cv.folds = 5)
gbm.perf(md, plot.it = TRUE)
```

It turns out that 105 trees are needed, which implies that decreasing in learning ratio results in more tree.

As the optimal number of trees is determined by depth and learning ratio, I use depth = 5 and learning ratio = 0.3 to build a GBM model, and the number of trees is 89 as cross-validation showed above..
```{r}
model_GBM =  gbm(trainSet$income ~ ., data = trainSet, distribution = "bernoulli",
          n.trees = 105, interaction.depth = 5, shrinkage = 0.3)
```

### 2.3.3 Performance of the model on test set.
The confusion matrix, auc and roc curve
```{r}
pred = predict(model_GBM,testSet,n.trees = 105)
table(ifelse(pred>0,1,0), testSet$income)
rocr_pred = prediction(pred, testSet$income)
...
plot(roc, colorize = TRUE)
abline(a = 0, b = 1)
table(ifelse(pred>0.-2,1,0), testSet$income)
```

With auc equal to 0.922 and a nice roc curve, it's easy to pick a cut off point around -2 to obtain a high TPR and low FPR, as shown in the last confusion table.


### 2.3.4 Explore the early stopping with h2o cluster.
TASK: use early stopping for determining the number of trees. Try also what happens if you don't use early stopping and you overtrain (increasing the number of iterations/trees).

Write out the data used in last section and upload it to h2o. Split the train and test data set.
```{r}
dx = h2o.importFile("/Users/beichensu/Documents/STAT418/Stat 418/wk-05-ML/HW3/mydata.csv")
dx$income = h2o.asfactor(dx$income)
dx_split = h2o.splitFrame(dx, ratios = 0.6, seed = 12)
...
Apply a GBM model without early stopping. Keep the learning rate at 0.3 and depth at 5, with large enough number of tree, 600.
```{r}
Xnames <- names(dx_train)[which(names(dx_train)!="income")]
time1 = system.time({
  md1 <- h2o.gbm(x = Xnames, y = "income", training_frame = dx_train, distribution = "bernoulli", 
                ntrees = 600, max_depth = 5, learn_rate = 0.3, nbins = 100, seed = 12)    
})
h2o.auc(h2o.performance(md1, dx_test))
time1
md1
```
Without any validation, the model employed 600 trees and gave auc at 0.9207 for the test data, with run time 26 second.

Now I employ the validation set to the model, use the AUC as the metric of stopping.

```{r}
time2 = system.time({
  md2 <- h2o.gbm(x = Xnames, y = "income", training_frame = dx_train, 
                 validation_frame = dx_valid, distribution = "bernoulli", 
                ntrees = 600, max_depth = 5, learn_rate = 0.5, nbins = 100, seed = 12, 
                stopping_metric = "AUC", stopping_rounds = 3, stopping_tolerance = 0.001)    
})
h2o.auc(h2o.performance(md2, dx_test))
time2
md2
plot(h2o.performance(md2))
```

With the validation set employed with early stopping option, the model only picked 18 trees and gave auc at 0.92313 with only 4.4 second. 

It turns out the the early stopping is a great feature that employs the validation set to avoid overfitting. It will give higher auc, and takes less run time.



# Discussion
On local machine, using auc of the metric of the goodnees, random forest fits the worst among three, but on h2o, they are roughly the same with same parameter.

The auc of them is really high, all above 0.85, which means that as we are investigating the TPR and FPR, we do have a nice choice of cut off points to obtain high TPR and low FPR. 

Aside those models, I feel that the predictors and the dependent variable in this data set are highly related so that we can gain a few machine learning model with high predictivity. As I tried on a show up vs no show up for medical appointment data set, the prediction is always bad, and the auc is even below 0.5, which shows me that those predictors such as height, weight and dicease type don't really affect they show up on their appointment or not. This makes sense in real life if I miss my appointment might becasuse I just forget about it, or I have to do something more important or the alarm doesn't ring. 

This comparision is educative that machine learning can't do much on uncoorelated variables actually.
